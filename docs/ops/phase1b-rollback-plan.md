# Phase 1.6B Rollback Plan: Write-Ahead Logging for Positions and Quotes

## Overview

This document describes the rollback plan for Phase 1.6B, which introduces write-ahead logging for positions fetch and quotes fetch operations. The feature is gated by the `write_ahead_logging` feature flag, which can be disabled to revert to the previous behavior.

## Rollback Criteria

Rollback should be triggered if any of the following conditions are met:

### 1. Duplicate Positions/Quotes Fetches Detected
- **Condition**: Duplicate positions or quotes fetches are triggered for the same symbols within a short time window.
- **Threshold**: 2 or more duplicate fetches within 5 minutes for the same symbol set.
- **Detection**: Monitor audit logs for duplicate `positions_intent` or `quotes_intent` events with the same symbol set.
- **Severity**: Critical - immediate rollback required.

### 2. Performance Degradation
- **Condition**: Positions or quotes fetch latency increases significantly compared to baseline.
- **Threshold**: Fetch latency > 10 seconds (baseline: < 2 seconds) for 5 consecutive fetches.
- **Detection**: Monitor positions/quotes fetch latency metrics.
...
### 4. Recovery Failures
- **Condition**: Recovery from crash fails or produces incorrect state for positions/quotes checkpoints.
- **Threshold**: Recovery action is incorrect (e.g., Resume when ManualReview is required) for 2 or more crash scenarios at positions/quotes checkpoints.
- **Detection**: Monitor recovery action accuracy in test runs.
- **Severity**: Critical - immediate rollback required.

### 5. Positions/Quotes Intent to Result Ratio Mismatch
- **Condition**: Audit log shows mismatched intent:result ratios for positions or quotes.
- **Threshold**: Intent:result ratio deviates from 1:1 by more than 10% for 5 consecutive runs.
- **Detection**: Monitor audit log validation reports.
- **Severity**: High - rollback within 1 hour.

### 6. Data Inconsistency
- **Condition**: Audit log state does not match broker state after recovery from positions/quotes checkpoints.
...
1. Run a dry-run to verify the feature is disabled:
   ...
   ```
2. Check the audit log to confirm no `positions_intent` or `quotes_intent` events are logged.
3. Verify positions and quotes fetch work without write-ahead logging.

### Step 4: Monitor for Issues
1. Monitor application logs for any errors.
2. Verify positions/quotes fetch latency returns to baseline.
3. Check that no duplicate fetches are triggered.
...
- [ ] Dry-run completed without `positions_intent` or `quotes_intent` events
- [ ] Positions and quotes fetch work without write-ahead logging
- [ ] Application logs show no errors
- [ ] Positions/quotes fetch latency returns to baseline
...
1. **Test Feature Flag Disable**
   ...
   - Confirm no `positions_intent` or `quotes_intent` events are logged.

2. **Test Positions/Quotes Fetch**
   - Fetch positions and quotes without write-ahead logging.
   - Verify fetches complete successfully.
   - Confirm audit logs are still written.

3. **Test Recovery**
   - Simulate a crash scenario at positions/quotes checkpoints.
   - Run recovery without write-ahead logging.
   - Verify recovery produces the correct action.

4. **Test Performance**
   - Measure positions/quotes fetch latency without the feature.
...
||-----------|----------|-------------------|-------------------|
|| Duplicate positions/quotes fetches | Critical | Immediate (< 5 min) | No (automatic) |
|| Recovery failures at positions/quotes | Critical | Immediate (< 5 min) | No (automatic) |
...
|| Intent:result ratio mismatch | High | < 1 hour | Yes (operator) |
...
3. Positions and quotes fetch work without write-ahead logging.
4. Positions/quotes fetch latency returns to baseline.
5. No duplicate fetches are detected.
6. Audit logs are still being written correctly.
7. Recovery works without write-ahead logging for positions/quotes checkpoints.
...
2. Consider manual intervention (e.g., manual position/quote verification).
