//! Tests for IBKR broker reconciliation blocking (F6 Phase 3).

#[cfg(feature = "ibkr")]
mod ibkr_reconcile_tests {
    ...
    #[test]
    fn test_reconciliation_blocks_on_orphan_order() {
        // This test verifies that when reconciliation is blocked,
        // order submission should fail with the appropriate error.
        // Since we can't actually connect to a broker in unit tests,
        // we verify the flag logic only.
        ...
        // Now reconciliation should be blocked
        assert!(broker.is_reconciliation_blocked());

        // Note: Actual submit_order() blocking is tested indirectly
        // through integration tests that require a real broker connection.
    }

    #[test]
    fn test_reconciliation_unblocks_after_clear() {
        let mut broker = IbkrBroker::new("127.0.0.1", 4002, 100);

        // Simulate orphan order detection
        broker.block_reconciliation();
        assert!(broker.is_reconciliation_blocked());

        // Simulate manual review and resolution
        broker.unblock_reconciliation();
